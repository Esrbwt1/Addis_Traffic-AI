    ...
    def optimize_traffic_lights(self):
        ...
        for tls_id in tls_ids:
            ...
            if max_queue_length > 10:
                # 1. Get the current state (e.g., "GrGr")
                current_state = traci.trafficlight.getRedYellowGreenState(tls_id)

                # 2. ONLY extend if the light is currently Green!
                # We look for 'G' (Green priority) or 'g' (Green minor)
                if "G" in current_state or "g" in current_state:
                    ...
                    # If Red, do nothing (wait for cycle to change)
                    pass
